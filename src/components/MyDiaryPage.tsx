    ...
    >
      ...
      <div className={cn("border-b p-3 flex flex-col gap-3 shadow-sm sticky top-0 z-50 overflow-visible transition-colors", isDarkMode ? "bg-[#111216] border-stone-800" : "bg-white border-stone-200")}>
        ...
        <div className="relative w-full">
          <div className={cn("rounded-2xl border bg-opacity-50 backdrop-blur-sm shadow-sm w-full overflow-x-auto touch-pan-x hide-scrollbar",
            isDarkMode ? "bg-stone-900/50 border-stone-800" : "bg-stone-50/50 border-stone-200")}>
            <div className="flex flex-nowrap items-center justify-start sm:justify-between gap-2 p-1.5 w-max min-w-full px-2">
              {/* Group 1: Core Tools */}
      ...
      <AnimatePresence>
        {(activeEditor || backgroundImage || activeTool === 'pen' || activeTool === 'eraser') && (
          <motion.div
            initial={{ y: -20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: -20, opacity: 0 }}
            className="bg-white/95 text-stone-800 backdrop-blur-md border-b border-stone-200 px-4 pt-2 pb-3 flex items-center justify-center gap-6 sticky top-[115px] z-40 shadow-sm overflow-x-auto touch-pan-x w-full hide-scrollbar"
          >
            <div className="flex items-center flex-nowrap w-max min-w-0 gap-6">
              ...
              {(activeTool === 'pen' || activeTool === 'eraser') && (
                <div className="flex flex-nowrap items-center gap-4 pr-6 border-r border-stone-200 last:border-0 last:pr-0">
                  <span className="text-[10px] font-bold text-stone-500 uppercase tracking-wider hidden sm:block shrink-0">{activeTool === 'eraser' ? 'Eraser' : 'Pen'}</span>
                  {activeTool === 'pen' && (
                    <div className="flex gap-1 items-center overflow-x-auto hide-scrollbar py-1 shrink-0">
                      {PEN_COLORS.map(c => (
                        <button
                          key={c}
                  ...
                  <div className="flex items-center gap-2 shrink-0 bg-white p-1.5 rounded-xl border border-stone-200 shadow-sm">
                    <span className="text-[10px] font-bold text-stone-400 mx-1">{activeTool === 'eraser' ? 'ERASER SIZE' : 'SIZE'}</span>
                    <input
                      type="range" min="1" max={activeTool === 'eraser' ? "100" : "25"} value={strokeWidth}
                      onChange={(e) => setStrokeWidth(Number(e.target.value))}
                      className="w-full max-w-[220px] sm:w-28 accent-stone-700 h-1 cursor-pointer"
                    />
                    <span className="text-[9px] font-mono text-stone-400 w-4 pl-1">{strokeWidth}</span>
                  </div>
                </div>
              )}
