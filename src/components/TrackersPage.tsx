...
function HealthTrackers() {
  ...
  return (
    ...
    >
      <section className="bg-[#FDFBF7] rounded-[32px] shadow-[8px_8px_0px_#D9D1C1] border-2 border-[#2C2C2C] p-8 relative overflow-hidden">
      ...
      <section className="bg-[#FDFBF7] rounded-[32px] shadow-[8px_8px_0px_#D9D1C1] border-2 border-[#2C2C2C] p-8 relative overflow-hidden">
        ...
        <div className="grid grid-cols-7 gap-1 mb-6">
          {['S','M','T','W','T','F','S'].map(d => (<div key={d} className="text-center text-[10px] font-black opacity-30 py-1">{d}</div>))}
...
function FinanceTracker() {
  ...
  const addExpense = async () => {
    ...
    await db.expenseLogs.add({ id: Math.random().toString(36).substring(2), date: format(new Date(), 'yyyy-MM-dd'), amount: parseFloat(amount), category, description: desc });
    setAmount(''); setDesc('');
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-12">
        ...
        {showAddBill && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white border-2 border-ink rounded-[32px] p-6 shadow-[8px_8px_0px_#D1D1D1]">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              ...
              <input type="text" placeholder="Due Day" value={bForm.dueDate} onChange={e => setBForm({...bForm, dueDate: e.target.value})} className="bg-cream rounded-xl p-3 text-sm font-bold border border-ink/5" />
      ...
      <div className="space-y-6">
        ...
        {showAddPolicy && (
          <motion.div initial={{ opacity: 0, height: 'auto' }} animate={{ opacity: 1 }} className="bg-white border-2 border-ink rounded-[32px] p-8 shadow-[8px_8px_0px_#D1D1D1]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {policies?.map(policy => (
            <div key={policy.id} className="bg-[#FDFBF7] rounded-[32px] border-2 border-ink p-6 shadow-[6px_6px_0px_#D1D1D1] relative group overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-4"><div className="p-3 bg-cream rounded-xl border border-ink/5"><span className="text-[8px] font-black uppercase opacity-40 block mb-1">Policy ID</span><span className="text-xs font-mono font-bold text-ink truncate block">{policy.policyNumber}</span></div></div>
            </div>
          ))}
        </div>
      </div>
